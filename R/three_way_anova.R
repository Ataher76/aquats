...
#'
#' This function performs a three-way ANOVA with full interaction effects, calculates
...
#' @param factor3_var Character; the name of the third categorical independent variable (facets).
...
three_way_anova <- function(data,
  ...
  if (!is.null(factor1_levels)) data[[factor1_var]] <- factor(data[[factor1_var]], levels = factor1_levels) else data[[factor1_var]] <- as.factor(data[[factor1_var]])
  if (!is.null(factor2_levels)) data[[factor2_var]] <- factor(data[[factor2_var]], levels = factor2_levels) else data[[factor2_var]] <- as.factor(data[[factor2_var]])
  if (!is.null(factor3_levels)) data[[factor3_var]] <- factor(data[[factor3_var]], levels = factor3_levels) else data[[factor3_var]] <- as.factor(data[[factor3_var]])

  # 2. Statistical Models
  formula_str <- as.formula(paste(numeric_var, "~", factor1_var, "*", factor2_var, "*", factor3_var))
  aov_model <- aov(formula_str, data = data)
  aov_summary <- summary(aov_model)
  tukey_res <- TukeyHSD(aov_model)

  # 3. Estimated Marginal Means & Letters for 3-Way Interaction
  emmean <- emmeans::emmeans(aov_model, specs = as.formula(paste("~", factor1_var, "*", factor2_var, "*", factor3_var)))
  ...
  sum_data <- data %>%
    dplyr::group_by(.data[[factor1_var]], .data[[factor2_var]], .data[[factor3_var]]) %>%
    dplyr::summarise(
  ...
  p <- ggplot(data, aes(x = .data[[factor1_var]], y = .data[[numeric_var]], fill = .data[[factor2_var]])) +
    theme_bw() +
    facet_wrap(as.formula(paste("~", factor3_var))) +
    labs(x = factor1_var, y = numeric_var, fill = factor2_var, title = "Three-Way ANOVA Interaction") +
  ...
  if (plot_type == "boxplot") {
    ...
    if (show_mean) {
      p <- p + stat_summary(fun = mean, geom = "point", shape = 18, size = 2.5,
    ...
    p <- ggplot(sum_data, aes(x = .data[[factor1_var]], y = mean_val, fill = .data[[factor2_var]])) +
      ...
      labs(x = factor1_var, y = numeric_var, fill = factor2_var, title = paste("Three-Way ANOVA (Mean \u00B1", toupper(error_type), ")")) +
      scale_fill_brewer(palette = color_palette) +
      theme(plot.title = element_text(hjust = 0.5), strip.background = element_rect(fill = "lightgray"))
    ...
    p <- ggplot(sum_data, aes(x = .data[[factor1_var]], y = mean_val, color = .data[[factor2_var]])) +
      ...
      labs(x = factor1_var, y = numeric_var, color = factor2_var, title = paste("Three-Way ANOVA (Mean \u00B1", toupper(error_type), ")")) +
  ...
  if (plot_type == "boxplot") {
    ...
    p <- p + geom_text(data = emmean_cld, aes(x = .data[[factor1_var]], y = max_y + y_nudge, label = .group, group = .data[[factor2_var]]),
                       position = position_dodge(0.8), inherit.aes = FALSE, size = 3.5)
  } else {
